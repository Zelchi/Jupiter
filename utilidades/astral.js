...
// Depositar - Astral
async function depositarAstral(message, argumentos, bancoDados) {
    ...
    const novoValor = Math.min(perfil.astral + valor);

    await perfilReferencia.update({ astral: novoValor });

    const embed = new EmbedBuilder()
        .setColor('#e0ffff')
        .setTitle('🎇 Ponto Astral recebido!')
        ...
        .setDescription(`${perfil.nomePersonagem} recebeu ${valor} moedas astrais. Atual: ${novoValor}.`)
        .setAuthor({ name: message.author.username, iconURL: message.author.avatarURL() })

    await message.channel.send({ embeds: [embed] });
    await message.delete();
};
export { depositarAstral };

// Pagar - Astral
async function pagarAstral(message, argumentos, bancoDados) {
    if (argumentos.length < 1 || argumentos.length > 2 || isNaN(argumentos[argumentos.length - 1])) {
    ...
    if ( valor > perfil.astral ) {
        await message.delete();
        return
    }

    const novoValor = perfil.astral - valor

    await perfilReferencia.update({ astral: novoValor });

    const embed = new EmbedBuilder()
        .setColor('#008b8b')
        .setTitle('🎇 Ponto Astral utilizado!')
        .setThumbnail('https://cdn.discordapp.com/attachments/1204870603446820945/1204870653677801472/Coin.gif?ex=66295cbd&is=66280b3d&hm=72bf5c9dea20153717964a1f9f16e81d4a1cce4370e01457184905e96eaff911&')
        .setDescription(`${perfil.nomePersonagem} utilizou ${valor} moedas astrais. Atual: ${novoValor}.`)
        .setAuthor({ name: message.author.username, iconURL: message.author.avatarURL() })
